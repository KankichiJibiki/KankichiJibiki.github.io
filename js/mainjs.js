'use strict'
  // *****----Beginning of Vocabraly----*****
{
  const vocaset = [
    {d:'1:May 5', 
    v:['Take a crack at','rant','grumpy','get s.t out of your system','date around'],
    m:['Attempt','愚痴(more than vent)','testy','やり尽くして卒業した様','いろんな人と付き合う']},
    {d:'2:May 6', 
    v:['Tacky','Testy','Vindictive','lemon',' mixed signal'],
    m:['ダサい(in a taste or fashion way)','Grumpy','潔白を証明する','不良品','あいまいな']},
    {d:'3:May 10', 
    v:['Not cut it','insolent','on a whole nother level','dedicated','glitch'],
    m:['informal to not be good enough to do something','cheeky','totally different','someone who is dedicated works very hard at what they do because they care a lot about it','out of order']},
    {d:'4:May 14', 
    v:['sprained something','scrape','insect bite','pimple','blister'],
    m:['Twisted','擦る','虫刺され','にきび','水ぶくれ']},
    {d:'5:May 16', 
    v:['gut (feeling/instinct/reaction)','get by','not count someone out','on the go all day','make ends meet'],
    m:['勘','やりくりする(お金面)','重要な人','一日中、動き回る','支出のやりくりをする']},
    {d:'6:May 17', 
    v:['thaw','get it together','on track','guts','-fold'],
    m:['溶かす','しっかりする','進行中','ガッツ','~倍']},
    {d:'7:May 18', 
    v:['mollify','do out of spite','high-maintenance','Its so someone','devote oneself to'],
    m:['to make someone feel less angry and upset about something SYN placate','腹いせに','手のかかる','~だれだれみたい','～捧げる']},
    {d:'8:May 21', 
    v:['make something out of something','bundle up  SYN wrap up','once in blue moon','snug ','dominant'],
    m:['','','','','']},
    {d:'9:May 22', 
    v:['elephant in the room','even as ','I am hearing/seeing things','cutlery','dominish'],
    m:['','','','','']},
    {d:'10:May 23', 
    v:['cranky (testy, grumpy)','flip out (go mad)','surreal ','blow off steam','in return'],
    m:['','','','','']},
    {d:'11:May 24', 
    v:['take something out on me','repel','snug','on the spot','put an end'],
    m:['','','','','']},
    {d:'12:May 25', 
    v:['affluent(wealthy)','you got me','tell it like it is','snug (tight)','prejudice'],
    m:['','','','','']},
    {d:'13:May 26', 
    v:['botch (up)','get away with murder','grant ','acute ','once and for all'],
    m:['','','','','']},
    {d:'14:May 27', 
    v:['shroud','explicit','disparity in/between','based on ~','prevail'],
    m:['','','','','']},
    {d:'15:May 28', 
    v:['hell bent (on)','trade off pv (for/against) ','tangle (up) ','twist (things)','peculiar'],
    m:['very determined to do something, especially something that other people do not approve of','~どちらを取るか','絡まる','ねじれる','strange/weird']},
    {d:'16:May 29', 
    v:["I'm torn between",'hook up','on the edge','intimate','tease'],
    m:['迷っている','いちゃいちゃする','ぎりぎりで・ on the verge of','親密な','からかう']},
    {d:'17:June 1', 
    v:["I'm still on the fence about",'hook up with ','bail on someone','vicious','timid'],
    m:['','','','','']},
    {d:'18:June 2', 
    v:['hook someone up with s.t / s.o','hurl (throw by force)','in spite of ',"I've been meaning to ",'suppose (assume)'],
    m:['','','','','']},
    {d:'19:June 6', 
    v:['Despite (the fact that) ','split up',"suppose (let's say)",'All but 2 something ','I found myself ~'],
    m:['','','','','']},
    {d:'20:June 7', 
    v:['in hot water','sloth (laziness) indolence','burgeon','uphold','benchmark'],
    m:['','','','','']},
    {d:'21:June 8', 
    v:['knock off','slouch','on thin ice','set in stone','impervious to'],
    m:['','','','','']},
    {d:'22:June 9', 
    v:['given','let off steam','inscrutable','sickening','keep someone company'],
    m:['考えられる','ストレス発散','謎めいた','きもちわるい','一緒にいる、come along']},
    {d:'23:June 10', 
    v:['burly','sissy ','detrimental to','scooch',"It's over my head"],
    m:['','','','','']},
    {d:'24:June 11', 
    v:['stand someone up ','hulk out','pull someone out of something','roundabout','nonchalant','cut to the chase'],
    m:['','','','','']},
    {d:'25:June 12', 
    v:['blatant','go right down the drain','off the top of my head','toxic','induce','ingratiant oneself'],
    m:['','','','','']},
    {d:'26:June 13', 
    v:['gingerly','diputise','Out of my hand','accordingly','come to think of it','give someone a nudge'],
    m:['','','','','']},
    {d:'27:June 15', 
    v:['correspnd','in a correspndence','supposedly','run through','blurt out','scrutinize'],
    m:['対応する','～と対応','（一般的に）言われている［思われている］','slip of the tounge','to examine someone or something very carefully']},
    {d:'28:June 16', 
    v:['fend for oneself','On a budget','Chew on s.t','Bug out','Deficiency '],
    m:['','','','','']},
    {d:'29:June 17', 
    v:['sketchy','P + the wee hours','photogenic','platitude','grapple with'],
    m:['vague','American English the early hours of the morning, just after 12 o’clock at night','','','']},
    {d:'30:June 18', 
    v:['at this ungodly hour','Squirm','Scoot (off)','With alacrity','Novelty'],
    m:['めっちゃ早朝','wriggle','急いで逃げる表現','brink','新鮮さ']},
    {d:'31:June 19', 
    v:['ever since','on and off','equivocal','complacency or complacent adj','clag up'],
    m:['','','','','']},
    {d:'32:June 20', 
    v:['fond of someone ','Pushover','Then and there','accrue','boggle ones mind'],
    m:['','','','','']},
    {d:'33:June 22', 
    v:['Plagiarism ','come back to bite me in the ass ','touch on','hit me','self-conscious'],
    m:['','','','','']},
    {d:'34:June 23', 
    v:['hit on ','rule out','Conceit','Circumlocution','I’m on a roll','Tiptoe around '],
    m:['','','','','']},
    {d:'35:June 24', 
    v:['Extravagant','Keep someone  mind off','The bottom line','Out of the blue','Abruptly','conspicuous'],
    m:['','','','','']},
    {d:'36:June 25', 
    v:['meddle','put someone on the spot','smell a rat','immerse oneself in something','dispirited','plausible'],
    m:['','','','','']},
    {d:'37:June 26', 
    v:['Intolerant','Incredulous','Sloppy',' I don’t really have a preference','Upbeat'],
    m:['1 not willing to accept ways of thinking and behaving that are different from your own','unable or unwilling to believe something','雑な','好みがない','OPP downbeat']},
    {d:'38:June 27', 
    v:['Keep your nose clean','Give/get the cold shoulder','Wear your heart on your sleeve','Keep a Stiff upper lip','Intractable ','Personable'],
    m:['厄介ごとに巻き込まれない','冷たい態度を取る','思ったことを正直に言う様','感情を隠す様','非常に困難な','someone who is personable is attractive and pleasant']},
    {d:'39:June 30', 
    v:['cover the cost with my own money','In a nutshell','have a thin skin','irresolute','drastic '],
    m:['実費','簡単に言うと','撃たれ弱い','wishy-washy','extreme and sudden']},
    {d:'40:July 1', 
    v:['have a thick skin ','To put it bluntly',"get under one's skin",'decrepit','Receptive'],
    m:['','','','','']},
    {d:'41:July 2', 
    v:['flatter myself that','let the cat out of the bag','think out of the box(think outside the box)','for just hell(heak) of it','Benevolently'],
    m:['','','','','']},
    {d:'42:July 3', 
    v:['Stall','Put someone up to something ','Has been better days','Bawl out','Efficacious '],
    m:['','','','','']},
    {d:'43:July 7', 
    v:['be all talk no action','old school','throw a fit','get carried away','tranquil','urbain'],
    m:['','','','','']},
    {d:'44:July 8', 
    v:['Sweep under the rug/ carpet','As though','Square up','Temperamental','Feasible ','Lousy'],
    m:['','','','','']},
    {d:'45:July 9', 
    v:['touch on','fair and square','follow suit','menacingly','lax'],
    m:['','','','','']},
    {d:'46:July 10', 
    v:['go with the flow','Follow through ','Not add up','Go over well','Equivocal '],
    m:['みんながやることの流れに任せる','やりきる','つじつまがあわない','うまくいく','ambiguous']},
    {d:'47:July 11', 
    v:['straightforward','heft','antagonize','Be in limbo','Get to do'],
    m:['','','','','']},
    {d:'48:July 12', 
    v:['harp on about ','pluck up courage',"have somr thing up one's sleeve",'preclude','expiate'],
    m:['','','','','']},
    {d:'49:July 13', 
    v:['To hell with ','Conspire ','First off','Flawless','Nominally ','Commonplace '],
    m:['','','','','']},
    {d:'50:July 14', 
    v:['Put~first','Get cold feet','Bold','At stake','Reticent','Jumbled '],
    m:['','','','','']},
    {d:'51:July 16', 
    v:['be far ooer','trigger','Seamless ','jumbled','lavish'],
    m:['','','','','']},
    {d:'52:July 17', 
    v:['Cocky','by the skin of my teeth','Flaky','Wary of','Capitulate ','Vanquish '],
    m:['','','','','']},
    {d:'53:July 21', 
    v:['susceptible','exponentially','indefinate','staggered','delineate','plausible'],
    m:['維持が可能な','2～4倍','不確定な、vague','extremely surprised SYN amazed','discribe precisely','~ありきたりなこと']},
    {d:'54:July 22', 
    v:['Reprimand ','Game-changing ','Boost','In the first place','Integrity '],
    m:['','','','','']},
    {d:'55:July 24', 
    v:['In the first place','stringent ','crystal ball','forgo','purvay'],
    m:['','','','','']},
    {d:'56:July 25', 
    v:['forgo','vigilant','shoplift','kill time','eschew'],
    m:['','','','','']},
    {d:'57:July 26', 
    v:['lenient','bothersome','thereby','trustworthy','worthwhile'],
    m:['','','','','']},
    {d:'58:July 28', 
    v:['worthwhile ','gruff', 'revert to','abide by','forthright' ],
    m:['','','','','']},
    {d:'59:July 30', 
    v:['viable','Buy time','relapse','exceptionally','shallow','waver'],
    m:['','','','','']},
    {d:'60:July 31', 
    v:['Keep something afloat','Reel v reeling(from) adj','Bustling with','Speculation ','Make a scean','exceptionally'],
    m:['','','','','']},
    {d:'61:Aug 1', 
    v:['Startle','Bust up','Say no more','Steadfast','Precarious ','come to light'],
    m:['to make someone suddenly surprised or slightly shocked','break up','これ以上言わなくていい','unstable','reveal']},
    {d:'62:Aug 24', 
    v:['Reticent ','Impede','Corny','Squander','Complacency/complacent'],
    m:['','','','','']},
    {d:'63:Aug 25', 
    v:['Light-hearted','Reticent','Adhere to','Boil down to ','My way around'],
    m:['','','','','']},
    {d:'64:Aug 26', 
    v:["put one's foot down",'corrode','plentiful','render something','ground situation/situation on the ground'],
    m:['','','','','']},
    {d:'65:Aug 27', 
    v:['readily','pernicious','in flux','aspiration','contingent'],
    m:['','','','','']},
    {d:'66:Aug 28', 
    v:['as a matter of fact','beats me','throw someone under the bus','be fond of someone','beef up'],
    m:['','','','','']},
    {d:'67:Sep 1', 
    v:['rat','pan out','pummel','flaunt','no strings attached'],
    m:['密告する','to happen or develop in a particular way','1 to hit someone or something many times quickly, especially using your fists (=closed hands) SYN beat','地位や金を見せびらかす','交換条件のない']},
    {d:'68:Sep 4', 
    v:['stamp out','to this day',"right under someone's nose",'hold someone accountable','grudgingly'],
    m:['','','','','']},
    {d:'69:Sep 5', 
    v:['lay bare','time frame','as hell','I can relate to something','Clutter'],
    m:['','','','','']},
    {d:'70:Sep 6', 
    v:['entail','futile','be accustomed to do','antithetical to something','On the flip side'],
    m:['','','','','']},
    {d:'71:Sep 7', 
    v:['frantic','rebellious','plow+ (Adv)','wealth of','Rocket science'],
    m:['','','','','']},
    {d:'72:Sep 8', 
    v:['Plow through',"(Don't) patronize someone",'rebellious','act up','be geared up for'],
    m:['','','','','']},
    {d:'73:Sep 9', 
    v:['paramount','circumvent','disregard','get up on the wrong side of the bed','be bogged down in'],
    m:['最重要','税金などを避けるさま','ingnore','in the bad mood/spirit','1 if a process or plan becomes bogged down, it is delayed so that no progress is made']},
    {d:'74:Sep 11', 
    v:['concoct','tough cookie','pleasently surprise','gut-wrenching','Agitate'],
    m:['','','','','']},
    {d:'75:Sep 13', 
    v:['buckle down to','fall out with','nod in agreement','perpetrate','eminent'],
    m:['真剣に取り組む','仲が悪くなる','首を縦に振らせる','commit','an eminent person is famous, important, and respected']},
    {d:'76:Sep 14', 
    v:['nod of approval','snazzy',"work in one's favor",'depraved','deficiency'],
    m:['首を縦に振る','粋な','有利に働く','very poor','~の不足(vitamin dificiency)']},
    {d:'77:Sep 15', 
    v:['be highly regarded','be viewed as','belligerant','worn on','stand someone up'],
    m:['高く評価されている','～として見られる','好戦的','as time wears on','bail on,blow off,']},
    {d:'78:Sep 17', 
    v:['Mitigate','Not to be outdone','Relentless','Undeterred','Shudder'],
    m:['','','','','']},
    {d:'79:Sep 18', 
    v:['Outdo ','Shudder','worrisome','Unwitting ','Beef about'],
    m:['～勝る','粉々になる','making you anxious','unknowing','愚痴をいう']},
    {d:'80:Sep 19', 
    v:['Outdo','In my defense ','I’ve been meaning to','Claw back','Out of my hand'],
    m:['～勝る','言っておくけど的な','～しようとずっと思ってた','to get back something that you had lost, by trying very hard','手に負えない']},
    {d:'81:Sep 20', 
    v:['Ease up on someone','For the long haul','Delve into','Hypocrite','Overstep'],
    m:['','','','','']},
    {d:'82:Sep 21', 
    v:['The moment has passed ','Sort out','Truancy ','Overthrow ','On par with'],
    m:['さっきやりたかったけどいまはもういいさま','Organize, tide up','Absent','1 to remove a leader or government from power, especially by force SYN oust','と肩を並べる']},
    {d:'83:Sep 22', 
    v:['Ebullient ','Deputize for something','Thumbs up','Consensual ','Have someone back'],
    m:['very happy and excited in a formal way','deligate for','agree with','同意のある','~味方、守る様']},
    {d:'84:Sep 23', 
    v:['Vengeful','Grill','Forthcoming ','Pay tribute to','Courageous '],
    m:['','','','','']},
    {d:'85:Sep 24', 
    v:['Maddening','Get laid ','On the fly','Spring up','Hail as'],
    m:['infurious','have sex','すぐに','advent',' to describe someone or something as being very good']},
    {d:'86:Sep 25', 
    v:['Topple','Internalize','Uptight','Think of the way that~','Nose around'],
    m:['','','','','']},
    {d:'87:Sep 26', 
    v:['Manhandle','Rack up','Nosy','Be preoccupied ','What do you make of'],
    m:['','','','','']},
    {d:'88:Sep 27', 
    v:["I can't unsee it",'sobriety','relent','deterrent (to)','fend for myself'],
    m:['','','','','']},
    {d:'89:Sep 30', 
    v:['Be revered as','Knack','Off the top of my head','Blatant','Revoke'],
    m:['be admired','コツ','いま思いつく限り','露骨に','却下、撤回']},
    {d:'90:Oct 1', 
    v:['Discrepancy ','Own up to','Eradicate','Baseless','Agitate','specuration'],
    m:['','','','','']},
    {d:'91:Oct 2', 
    v:['Weed out','Crack down on','Undermine','Grasp at strew','Mellow'],
    m:['','','','','']},
    {d:'92:Oct 3', 
    v:['Stamp out','Prostitute ','Talk someone down','Account for','After the dust settles'],
    m:['撲滅する','売春婦','説得する','～占めている','～が終わったら、落ち着いたら']},
    {d:'93:Oct 5', 
    v:['Radical','Platonic','Comfort zone','Geek out over','Pair up'],
    m:['a radical change or difference is very big and important OPP slight','a relationship that is platonic is just friendly and is not a sexual relationship','楽なゾーン','好きなことをめちゃくちゃ喋ること','ペアを組む']},
    {d:'94:Oct 6', 
    v:['Clean freak','That’s more like it','Blast AC / heater','Backhanded compliment ','Unsanitary','pull someone out of something'],
    m:['','','','','']},
    {d:'95:Oct 7', 
    v:['Induce','Cursory','pull someone out of something','Dead pan face','Have an affinity for'],
    m:['','','','','']},
    {d:'96:Oct 8', 
    v:['inquisitive','Nail someone down to ','Rings a bell','Blank out','Judge a book by its cover'],
    m:['','','','','']},
    {d:'97:Oct 9', 
    v:['Nail  down','curtly','Take to someone’s ears off','flabbergusted','Blow money'],
    m:['1. to reach a final and definite agreement or decision about something 2.nail somebody down to force someone to say clearly what they want or what they intend to do','using very few words in a way that seems rude','一歩的に話す','仰天する','squander']},
    {d:'98:Oct 11', 
    v:['seem off','introvert','across the board','lousy','Slack off'],
    m:['','','','','']},
    {d:'99:Oct 12', 
    v:['extravert','change the tide','a dog with the bone','cut corners','come clean','consume'],
    m:['','','','','']},
    {d:'100:Oct 13', 
    v:['vanilla','feign','uproot','rest up for','go for'],
    m:['普通、つまらない','to pretend to have a particular feeling or to be ill, asleep etc','上京する','～のために休む','cost for']},
    {d:'101:Oct 14', 
    v:['This is something','prelude to ','cramped','unbearable','brown away','retain'],
    m:['すごい','～の前触れ','せまい','耐えられない','感動、衝撃をうけた','to keep something or continue to have something']},
    {d:'102:Oct 15', 
    v:['resort to ','refer to ','lively','stick around','smooth sailing','pitch in'],
    m:['～手段を取る','look at/ be called','にぎやかな','ぶらぶらする','順風満帆','協力する']},
    {d:'103:Oct 17', 
    v:['just because','throw s.o off','spill the beans','opinionated','stick it out'],
    m:['','','','','']},
    {d:'104:Oct 18', 
    v:['tough it out','revamp','Keep someone mind off','Come on too strong ','Be a world away from '],
    m:['やり抜く、頑張りぬく','改装、改造','～から話をそらす、気を散らす','強引に','～と世界が違う']},
    {d:'105:Oct 19', 
    v:['dont knock it until you try','Softie','Rave about','Opt out','Self control '],
    m:['実際にするまで否定するな','情にもろい人','to talk about something you enjoy or admire in an excited way','手を引く','自制心']},
    {d:'106:Oct 20', 
    v:['be intent on something','get to the point','get out of the woods','swing by','remorse'],
    m:['決意する','要するに','困難を乗り越える','～少し寄る','深い後悔']},
    {d:'107:Oct 21', 
    v:['contradictory','in this day and age','swamped','back in the day','hold in'],
    m:['矛盾してる','今日この頃、最近','be inundate with','昔、さかのぼると','bottle up']},
    {d:'108:Oct 22', 
    v:['bottle up','parameter','drop it','hammer out','patch up'],
    m:[' to deliberately not allow yourself to show a strong feeling or emotion / SYN hold in','限界、','やめる、失敗する','案出する','パッチ作業、応急手当']},
    {d:'109:Oct 23', 
    v:['seamless','at whim','go over well','go with the flow','spare no expence'],
    m:['','','','','']},
    {d:'110:Oct 24', 
    v:['arbitrary','in passing','tell me about a time when','antagonize','tantamount'],
    m:['独断的な','通りすがりに','','反感を買う','if an action, suggestion, plan etc is tantamount to something bad, it has the same effect or is almost as bad']},
    {d:'111:Oct 26', 
    v:['immeasurable','depict','evasive','debacle','ghastly'],
    m:['','','','','']},
    {d:'112:Oct 27', 
    v:['groundless','unduly','strenuous','exasperate','mingle with'],
    m:['','','','','']},
    {d:'113:Oct 28', 
    v:['infer','flat out','immaculate','get around to ','go with'],
    m:['','','','','']},
    {d:'114:Oct 29', 
    v:['stir up','posthumously','rile up',"can't hurt",'reiterate'],
    m:[' to deliberately try to cause arguments or bad feelings between people','死後に','informal to make someone extremely angry','～しても問題ない','repeat']},
    {d:'115:Oct 30', 
    v:['rebut','lay it on me','murky','agitate','the jury is still out'],
    m:['','','','','']},
    {d:'116:Oct 31', 
    v:['come off (as)','talk over someone','in the back of my head','go overboard','pervasive'],
    m:['come accross (as)','誰がの話をさえぎって話す','脳裏では、どこかでは、うすうす','調子にのる、やりすぎる','蔓延している']},
    {d:'117:Nov 1', 
    v:['jump in',"get one't feet wet",'while you are at it','No strings attached','cocky'],
    m:['interapt','足を踏み入れる','ついでに','交換条件なしに','天狗']},
    {d:'118:Nov 2', 
    v:['roll out','plug away at','play it by ear','align with','digress'],
    m:['','','','','']},
    {d:'119:Nov 3', 
    v:['establish oneself as','iron out','in my book','pervasive','dodge'],
    m:['～として地位を確立する','to solve or get rid of problems or difficulties, especially small ones',' said when giving your opinion','～蔓延する','～かわす、parry']},
    {d:'120:Nov 4', 
    v:['rip off','mediocer','wade in/through','get no service','extenuating circumstances'],
    m:['','','','','']},
    {d:'121:Nov 5', 
    v:['govern','sizeable','silver lining','blab to','quote unquote'],
    m:['','','','','']},
    {d:'122:Nov 6', 
    v:['takeaway','level with','trespass','from day to day','complicity'],
    m:['','','','','']},
    {d:'123:Nov 7', 
    v:['variaty of','Fallback','go through the roof ','disastrous','glorify'],
    m:['','','','','']},
    {d:'124:Nov 9', 
    v:['if you ask me','pick up on','with that being said','at (Time) sharp','laid-back adj'],
    m:['言うならば','気が付く','そうは言ったものの/ということで','ちょうど～時','落ち着いている']},
    {d:'125:Nov 10', 
    v:['fall back on','perpetrate','outside of','honorific','elucidate'],
    m:['頼る','commit','~以外に','敬語','clarify']},
    {d:'126:Nov 11', 
    v:['valger','feel someone out','drop the F bomb','unapologetic','pragmatic'],
    m:['','','','','']},
    {d:'127:Nov 12', 
    v:['stave off','fastedious','by all means','fly off the handle','belittle'],
    m:['','','','','']},
    {d:'128:Nov 13', 
    v:['overreach oneself','it behooves somenone to do something','doozy ','dubious','bark up the wrong tree'],
    m:['無理をする、権限・資金などを超えて','formal used to say that someone should do something because it is right or necessary, or it will help them','something that is extremely good, bad, strange, big etc','doubtful,fishy,shady','見当違いのものを追う']},
    {d:'129:Nov 14', 
    v:['cagey','stash away','on the same page','spit it out','kiss up to '],
    m:['','','','','']},
    {d:'130:Nov 15', 
    v:['Back in','silver bullet','dry up','read s.t/s.o','abhor'],
    m:['','','','','']},
    {d:'131:Nov 16', 
    v:['grubby','impertial','two-faced','stranded','concede'],
    m:['','','','','']},
    {d:'132:Nov 18', 
    v:['dauting','aboveboard','rotate','call in sick','tweak'],
    m:['くじけそうになる何か','honest and legal 公明正大な','ローテーション','病欠する','to make small changes to a machine, vehicle, or system in order to improve the way it works']},
    {d:'133:Nov 19', 
    v:['distractedly','imterminable','leeway','throw out something','go the extra miles '],
    m:['anxious and unable to think clearly','endless',' freedom to do things in the way you want to','to make someone leave a place, school, or organization, especially because they have done something that is against the rules','make effort more']},
    {d:'134:Nov 20', 
    v:['crystal clear','flurry','wrap s.t up','on pins and needles',"I'm booked solid"],
    m:['','','','','']},
    {d:'135:Nov 21', 
    v:['cohesive','insidious','bend over backwards','fead','the apple of ones eyes'],
    m:['','','','','']},
    {d:'136:Nov 22', 
    v:['suave','bum out','wanton','ground yourself','right as rain '],
    m:['','','','','']},
    {d:'137:Nov 25', 
    v:['exhaustive','a little birdie told me','work in ones havor','flat-out','in a bind'],
    m:['extremely thorough and complete','風のうわさで','～の有利に働く','straght out','in trable']},
    {d:'138:Nov 26', 
    v:['expediant','intuitive','it just so happens that','go about','if anything','Trustworthy'],
    m:['都合のいい','instinctive','たまたま～してる','～を取り掛かる・やる・する','信用に値する']},
    {d:'139:Nov 27', 
    v:['pretentious','throw in the towel','indiscreminate','a big cheese','outlast'],
    m:['','','','','']},
    {d:'140:Nov 28', 
    v:['resort to ','milestone','seemingly','do a double take','rig'],
    m:['to do something bad, extreme, or difficult because you cannot think of any other way to deal with a problem','画期的なこと','apearently','二度見','仕掛ける']},
    {d:'141:Nov 29', 
    v:['downplay','hairy','endeavor','single out ','procure'],
    m:['','','','','']},
    {d:'142:Nov 30', 
    v:['freeze up','obtuse','lucrative','tame','fume about'],
    m:['','','','','']},
    {d:'143:Dec 1', 
    v:['assemble','loosen up',"Now you're talking",'obliterate','egg someone on something'],
    m:['','','','','']},
    {d:'144:Dec 2', 
    v:['out of juece','at the age of','take s.t back','pop up','presumptuous'],
    m:['充電きれ','～歳の','撤回する','come up ','図々しい']},
    {d:'145:Dec 3', 
    v:['replenish','subject to something','brand s.o as','blow over','invincible'],
    m:['','','','','']},
    {d:'146:Dec 4', 
    v:['block out','lurid ','does not seem right','prank',"make s.o sick to one's stomache"],
    m:['','','','','']},
    {d:'147:Dec 5', 
    v:['from that time forward/on','hop on','lag behind','(feel)sick to ones stomache','be underway'],
    m:['それから～','cab・taxiに飛び乗る','遅れる','feel like throwing up','進行中']},
    {d:'148:Dec 6', 
    v:['(have/hit/go through) a dry spell','before long','nose out','abet','long stretch'],
    m:['うまくいかない期間','すぐに','僅差で勝利','to help someone do something wrong or illegal','for a long time']},
    {d:'149:Dec 7', 
    v:['claw back','fall back on','one way or the other','on a regular basis','beat around the bush'],
    m:['ゆっくり戻る','頼る','either way','定期的に','回りくどくいう']},
    {d:'150:Dec 8', 
    v:['knuckle down','keep after','pull a prank on s.o','clear ones mind','hint at'],
    m:['真剣に取り組む','push for','ドッキリを仕掛ける','頭を整理する','imply about']},
    {d:'151:Dec 9', 
    v:['get s.t across to s.o','pin down','trick','few and far between in','off-limits'],
    m:['','','','','']},
    {d:'152:Dec 10', 
    v:['off-hours','keep to oneself','see the light at the end of the tunnel','facetious','you would think'],
    m:['','','','','']},
    {d:'153:Dec 11', 
    v:['pass off s.t as s.t','lay it on thick','hold up','have come to like s.t','give the green light'],
    m:['~をなになにとして偽る','exaggerate','Sorry I’m late – I was held up at work.& hold on','have grown to do','許可を与える']},
    {d:'154:Dec 12', 
    v:['opt for','kick up','(go/take/choose)route',"that's the story of ones life",'toasty'],
    m:['','','','','']},
    {d:'155:Dec 13', 
    v:['have grown to do ','get s.t down','crank up','doing binge(do on a binge)','latecomer'],
    m:['だんだん～してくる','コツをつかむ','おかしくする(おもしろくて)','一気に～する','遅れてくる人']},
    {d:'156:Dec 14', 
    v:['(get in )rabbit hole','hungry for','up-and-coming','in/into mode','cliche'],
    m:['息詰まること','~に対してハングリー精神が豊富','promising','~モード','常套句']},
    {d:'157:Dec 15', 
    v:['block out','long shot','give or take','a good (time)','procrastinate'],
    m:['','','','','']},
    {d:'158:Dec 16', 
    v:['wierdly/strangely enough','groggy','run in the family','dawdle','demean'],
    m:['','','','','']},
    {d:'159:Dec 17', 
    v:['my thing','think it through/think through it','dampen (down)','break the circle','set back'],
    m:['好み','よく考える',' to make something such as a feeling or activity less strong','悪い習慣を断ち切る','to delay the progress or development of something, or delay someone from finishing something']},
    {d:'160:Dec 21', 
    v:['deviate','queasy','before you know it','dole out to','time of day'],
    m:['逸れる','feeling that you are going to vomit','すぐに','to give something such as money, food, advice etc to more than one person','時間帯']},
    {d:'161:Dec 22', 
    v:['not give s.o time of day','underscore','take s.t with a grain of salt','rig','steer clear of'],
    m:['','','','','']},
    {d:'162:Dec 23', 
    v:['detract from','alleviate','overcast','in a/ get a/ a)running start','now that you mantion it'],
    m:['評価などが損なう','to make something less painful or difficult to deal with','dark with clouds','良い出だし、いい始まり','あ～いま言っていること分かったわｗ']},
    {d:'163:Dec 26', 
    v:['entice','in a matter of days','hold back','lead up to','draw back'],
    m:['ひきつける','数日間','留年','~に至るまで','あとずさりして(引き返す)']},
    {d:'164:Dec 27', 
    v:['ubiquitous','disparage','pick up','besides','pack up and go'],
    m:['','','','','']},
    {d:'165:Dec 28', 
    v:['push back','resonate','get in habit of','recap','suit yourself'],
    m:['日付を延ばす','~に響く','習慣を身に着ける','まとめ、まとめる','好きにしろ']},
    {d:'166:Dec 29', 
    v:['push forward','over-the-top','Rub off on','(get) stale','goal-oriented'],
    m:['日付を前倒しにする','盛る','良い影響を与える',' air that is stale is not fresh or pleasant OPP fresh','目標志向']},
    {d:'167:Dec 31', 
    v:['action-oriented','as it stands','use time wisely','requisite','get the picture'],
    m:['','','','','']},
    {d:'168:Jan 6', 
    v:['spill the beans','detail-orianted','paper over','in the maiking','emancipate'],
    m:['','','','','']},
    {d:'169:Jan 7', 
    v:['go by ','kill the mood','red flag','badger','debunk'],
    m:['','','','','']},
    {d:'170:Jan 8', 
    v:['acquire ','have an affinity','leave behind','there is no point in','in ones presence'],
    m:['','','','','']},
    {d:'171:Jan 10', 
    v:['hoax','hard up','make ends meet','mind-boggling','Vernacular'],
    m:['作り話','be broke','やりくりする','唖然とさせる / びっくりさせる(びっくりして信じられない様子)','その地域の言葉・世代間の言葉']},
    {d:'172:Jan 13', 
    v:['ostensible','adamant','commensurate','succinct ','dissent '],
    m:['表向きの','頑固','～に釣り合った、見合った','concise','consent']},
    {d:'173:Jan 16', 
    v:['throw around','heedless','gullible','quick to do','mindset'],
    m:['(言葉が)よく使われている','不注意な','騙されやすい','すぐに・早い(何かをするのが)','マインド']},
    {d:'174:Jan 17', 
    v:['win over','frugal','erroneous','wear ones heart on ones sleeve','think back on'],
    m:['誰かを味方につける','thrifty','incorrect','感情を率直に表す','look back to/on']},
    {d:'175:Jan 18', 
    v:['derogatory','candid','sassy','from place to place','get out of ones way to do'],
    m:['軽蔑的な','率直な','生意気(cheeky)/活発的な','場所による','わざわざ道をそれて～をする']},
    {d:'176:Jan 19', 
    v:['above and beyond','nonchalant','spurious','treachrous','cringy'],
    m:['期待を超える','平然と','胡散臭い','不誠実な','イタイ']},
    {d:'177:Jan 20', 
    v:['devious','lethergic','retrieve','to some extent','bright and early'],
    m:['treacherous','slugish','get back','ある程度','早朝']},
    {d:'178:Jan 21', 
    v:['erratic','subdued','in the cards','dont hold your breath','high end s.t'],
    m:['不規則な','空気がよどんだ','likely to happen','期待しないで','高級な・高性能な']},
    {d:'179:Jan 22', 
    v:['heed','reminisce','perfunctory','think straight','be accustomed to'],
    m:['耳を傾ける','思い出にふける','おざなりな','まともに考える','習慣やライフスタイルなどに慣れている']},
    {d:'180:Jan 23', 
    v:['ameliorate','harsh','switch over to','one-shot','counter intuitive'],
    m:['improve','厳しい','切り替える','one-off','直観に反すること']},
    {d:'181:Jan 26', 
    v:['tied to','play dumb','likelier','drench','impair'],
    m:['','','','','']},
    {d:'182:Jan 27', 
    v:['get a taste of','overstate','revise','grimy','communal'],
    m:['~が何なのか味わう、触れる',' exaggerate','改定する to change something because of new information or ideas','filthy','共同の']},
    {d:'183:Jan 28', 
    v:['transpose','embody','get the hang of','(have)a chip on ones shoulder','backlash against/from '],
    m:['formal to change the order or position of two or more things','体現させる','get used to','反発的な態度を取る','反発']},
    {d:'184:Jan 29', 
    v:['fit in','transcend','funky','on the fly','has something for everyone'],
    m:['','','','','']},
    {d:'185:Jan 30', 
    v:['hinder','over the course of','saturate','partial','hail down a cab'],
    m:['','','','','','']},
    {d:'186:Jan 31', 
    v:['breath-taking','amplify','a grown up','run on no sleep','bombard'],
    m:['息をのむほど素晴らしい','詳述','成長した大人','寝ていないで何かをやる','to do something too often or too much, for example criticizing or questioning someone, or giving too much information']},
    {d:'187:Feb 1', 
    v:['a solid','Status quo','jostle','way around','cater to','ambivalence'],
    m:['a good(最低でも～くらい)','the state of a situation as it is','押し合う','方法','～のニーズを満たす','hestation']},
    {d:'188:Feb 2', 
    v:['which leads me to my next question','assemble','plot','sharp','But for'],
    m:['これが次の質問につながるんだけど・・・','組み立てる','計画、conspire','smart, look great','If it were not for']},
    {d:'189:Feb 3', 
    v:['that about covers it','discreminatory','fraction of','bizzrre','in one go'],
    m:['','','','','']},
    {d:'190:Feb 5', 
    v:['all about','drawback','doable','put in work','leave up to'],
    m:['','','','','']},
    {d:'191:Feb 6', 
    v:['compunction ','know one way around','(put oneself)in my shues','talk around','push for'],
    m:['conscience','～周辺を知ってる','～の身になる','beat around the bush','keep after']},
    {d:'192:Feb 7', 
    v:['contradictory','on ones case','on paper','expansively','de facto'],
    m:['','','','','']},
    {d:'193:Feb 8', 
    v:['gratified','dish out','capricious','be on top of','walk the walk'],
    m:['','','','','']},
    {d:'194:Feb 9', 
    v:['egregious','copious','pertinent','that way','talk the talk','walk the walk'],
    m:['','','','','']},
    {d:'195:Feb 10', 
    v:['consequent','illicit','keep on the low','ferocious',"meet someone's expectation"],
    m:['','','','','']},
    {d:'196:Feb 11', 
    v:['frail','perks','look through','redundant','significant amount of'],
    m:['','','','','']},
    {d:'197:Feb 12 ', 
    v:['versatile','paid time off','lenient','love-hate-relationship','lurch'],
    m:['','','','','']},
    {d:'198:Feb 14', 
    v:['bank of question','cram','pick up','volatile',"meet one's expectation"],
    m:['','','','','']},
    {d:'199:Feb 15', 
    v:['pass the time','shy away from','middle of nowhere','whip up','start out with'],
    m:['','','','','']},
    {d:'200:Feb 16', 
    v:['devalue','a hit or miss','it slipped my mind','up until','put/lay/place emphasis on','gung-ho'],
    m:['to make someone or something or currency-wise seem less important or valuable','当たりはずれ','ド忘れ','until','～を強調する','熱心に何かをやる']},
    {d:'201:Feb 18', 
    v:['evade','demean','essentially ','made for ','get bent out of shape'],
    m:['avoid','to do something that makes people lose respect for someone or something → degrade','basiclly','cut out for','in the bad mood']},
    {d:'202:Feb 19', 
    v:['hassle','tenacious','imprudent',"say what's on ones mind",'off the cuff'],
    m:['such a pain in the neck','粘り強い','軽率な','speak up your mind','an off-the-cuff remark, reply etc is one that you make without thinking about it first']},
    {d:'203:Feb 22', 
    v:['a fine line between','spirit','prudent','take a toll on','speak ones mind'],
    m:['','','','','']},
    {d:'204:Feb 23', 
    v:['pushover','blemish','walk over someone','hold back','back down'],
    m:['','','','','']},
    {d:'205:Feb 24', 
    v:['get a head','fizzle out','brisk','age old','for reason'],
    m:['get promoted','だんだん消えていくさま','きびきびした noun','古き良き','理由があって']},
    {d:'206:Feb 25', 
    v:['religiously','come down to','cynicle','keep food on the table','unblemished'],
    m:['定期的に、宗教で週一回日曜日にcharchに行くようなイメージ','~次第・かかっている','天邪鬼、ひねくれている','養っていく','without marks or damage']},
    {d:'207:Feb 26', 
    v:['fad','aside from','conscription','get/be shufted','(put/pour)blood, sweat and tears','corporate people'],
    m:['流行り','as well as/besides','兵役制度','不当な扱い、ひどい扱われ方','その身をささげるくらいの努力てきな','出世意欲のあるひと']},
    {d:'208:Feb 27', 
    v:['come from ~ background','be burned out','thrive','live by','uncalled for'],
    m:['','','','','']},
    {d:'209:Mar 3', 
    v:['depletion','it just goes to show','on the edge of my seat','per se','flop','in the mood for'],
    m:['','','','','']},
    {d:'210:Mar 4', 
    v:['sit through','what goes around, comes around','put s.t on hold','cat fish','interfere'],
    m:['','','','','meddle']},
    {d:'211:Mar 6', 
    v:['daunting','toss around','model after','throwback to','extra hands'],
    m:['','','','','']},
    {d:'212:Mar 7', 
    v:['fantasize','blow off','be stuck on','abruptly','leave s.t hanging'],
    m:['','','','','']},
    {d:'213:Mar 8', 
    v:['go a long way','hands-on','rerun','compulsory','hand off'],
    m:['大いに役立つ','実践的な','再放送','Mandatory','仕事を誰かに割り振ったり引き継がせることを表し、特に上司が部下に仕事を任せる状況']},
    {d:'214:Mar 10', 
    v:['intrepid','distract','mam and dad shop','slack off','iconic'],
    m:['brave','気が散る','夫婦で営む店','だらける・さぼる','省庁象徴']},
    {d:'215:Mar 11', 
    v:['bland','make money off','pay cut','inadequate','stick with'],
    m:['','','','','']},
    {d:'216:Mar 12', 
    v:['phenomenal','add up to','no less than','the bottom line','trunout','millenial'],
    m:['','','最低でも(大きい値)','','','']},
    {d:'217:Mar 15', 
    v:['balance out','on occasion','be set on','probationary period','it has nothing to do with'],
    m:['均等に、バランスよくする','たまに','decide to','見習い期間','関係ない']},
    {d:'218:Mar 16', 
    v:['pet peeve','downturn','rush','ingrained in','six feet under','bond with'],
    m:['','','','','']},
    {d:'219:Mar 17', 
    v:['drug off','strike up a conversation','make the best of','sleazzy','I would rather not'],
    m:['','','','','']},
    {d:'220:Mar 20', 
    v:['hit it off with','petty','marginalize ','crack up','identify with'],
    m:['','','','','']},
    {d:'221:Mar 21', 
    v:['dwell on','bury ones in','bare minimam','fall into place','bubble over with'],
    m:['','','','','']},
    {d:'222:Mar 22', 
    v:['fend off','mingle with','be obliged to','add up','head off'],
    m:['','','','','']},
    {d:'223:Mar 23', 
    v:['fling','change over','overcharge','pin down','crop up'],
    m:['','','','','']},
    {d:'224:Mar 24', 
    v:['dote on','spooky','smooth down','hungout','soak up'],
    m:['','霊的に不気味','calm down','','']},
    {d:'225:Mar 25', 
    v:['perk up','shape','harninger','who am I to do','streetsmart'],
    m:['cheer up','','','','']},
    {d:'226:Mar 26', 
    v:['cart s.o off to ','chunk of','cheesy','trivialize','lament'],
    m:['誰かを～へ連行する','大きい・塊の','corny/lame','make s.t less important','嘆く']},
    {d:'227:Mar 27', 
    v:['play down','innate','hypotheticly speaking','along those lines','make or break','just because'],
    m:['','','','','']},
    {d:'228:Mar 28', 
    v:['squeeze in','biased','get ones hopes up','corny','attentive'],
    m:['忙しい予定に割入れる','unfairly preferring one person or group over another','期待する','くさい、ださい','1 listening to or watching someone carefully because you are interested/2 making sure someone has everything they need']},
    {d:'229:Mar 29', 
    v:['think along the same lines','potent','fickle','make a habit of','the reason being that'],
    m:['そのように考える','having a very powerful effect or influence on your body or mind SYN powerful',' someone who is fickle is always changing their mind about people or things that they like, so that you cannot depend on them – used to show disapproval','～習慣をつける','～の理由は']},
    {d:'230:Mar 30', 
    v:['put it that way','gather oneself','genetic',"get ones money's worth",'stick it out'],
    m:['','','','','']},
    {d:'231:Mar 31', 
    v:['vouch for','embezzle','push down ones throat','bust out','reach a decision'],
    m:['証明する','横領','～を押し付ける','burst out~に～しだす','決定に至る']},
    {d:'232:Apr 1', 
    v:['pull oneself together','be spured on','wind down','spraut up','rev up'],
    m:['gather yourself','be motivated','休む','bergion, come up','if you rev up a system or organization, or if it revs up, it becomes more active']},
    {d:'233:Apr 2 ', 
    v:['weigh in','trump up','different ball game','on the move','perfect v'],
    m:['仲裁、割り込む','偽造する','totally different situation','on the go','Verb ver perfect']},
    {d:'234:Apr 3 ', 
    v:['no joke','skim off','wind up','good value for ones money',"that's very someone to do"],
    m:['','','','','']},
    {d:'235:Apr 4', 
    v:['buy into','sponge off','stand in for',"I'll give you that but...",'blast'],
    m:['信じない','すねをかじる','～代役を買って出る','それは認めるけど～','']},
    {d:'236:Apr 6', 
    v:['pomper','tick off','a whole new game',"all over the place",'bring out the best'],
    m:['','','','なんでも～するよ(どんな曲でも聞くよ的な)','']},
    {d:'237:Apr 8', 
    v:['high-strung','troll','side effect',"dawn on me that",'self-esteem'],
    m:['','','','','']},
    {d:'238:Apr 9', 
    v:['perception','more than happy','have a soft spot for ',"pop up into ones mind/head",'spoiler'],
    m:['','','','','']},
    {d:'239:Apr 10', 
    v:['get s.o in groove','aptitude','reserved',"flaky",'on the bright side'],
    m:['','','','','']},
    {d:'240:Apr 12', 
    v:['ulterior motive','creep s.o out','sentimental',"rampant",'(fly) under the rader'],
    m:['','','','','']},
    {d:'241:Apr 13', 
    v:['haggle','buckle down','as far as ~ goes',"pick ones up",'next big thing'],
    m:['','','','cheer up','fad']},
    {d:'242:Apr 14', 
    v:['no-go','cover ones ear','guilty peasure',"more or less",'signiture s.t'],
    m:['not allowed and unacceptable','','','','']},
    {d:'243:Apr 15', 
    v:['as far as im concerned','scramble','look to',"you cant go wrong with",'hole in the wall'],
    m:['','','rely on','','']},
    {d:'244:Apr 16', 
    v:['carse','out of sorts','backlog',"fall on",'oldie'],
    m:['','','','','']},
    {d:'245:Apr 17', 
    v:['equivalent','in the grand scheme of things','cover',"reconcile",'repercussion'],
    m:['~と同等な','長い目でみれば、俯瞰的に見れば','bring up 話題に挙げるさま','compromise','影響']},
    {d:'246:Apr 22', 
    v:['in/with hindsight','conjecture','massive',"chenge out of something (into)",'vibrant'],
    m:['今にして思えば, 後になってわかったが','推測','big','～に着替える','活気のある']},
    {d:'247:Apr 23', 
    v:['dazed ','excessive','profanity',"meaning behind",'pump out'],
    m:['唖然としている','過度の','Curse','～意味の裏に','1 if something such as music, information, or a supply of products pumps out, or if someone pumps it out, a lot of it is produced']},
    {d:'248:Apr 27 ', 
    v:['commensurate with','go up to s.o','so-and-so',"put it to use",'only ~ % of the battle'],
    m:['~と釣り合った','approch to s.o','匿名のだれだれ～','使用する、応用する','全体の~%にしか過ぎない']},
    {d:'249:Apr 30', 
    v:['disgrintled','be half the battle','exquisite',"be frowned upon",'straightforward'],
    m:['不満な','～大変な','絶妙な','顔をしかめつけた、賛成しないさま','ド直球']},
    {d:'250:May 1', 
    v:['dissident','if you dont mind me asking','sway',"clueless about",'noteworthy'],
    m:['意見の不賛成、不一致','差し支えなければ～',' to influence someone so that they change their opinion','まったくわからない','注目すべきもの']},
    {d:'251:May 3 ', 
    v:['slur','condusive','you can bet on that',"not long after",'means'],
    m:['呂律が回らない','貢献する','間違えない','すぐに・まもなく','方法']},
    {d:'252:May 5 ', 
    v:['have an urge','intense','incessant',"To my knoledge",'Plan B'],
    m:['衝動的に何かがしたくなる','having a very strong effect or felt very strongly','絶え間なく','As fer as I know','Back up plan']},
    {d:'253:May 7 ', 
    v:["If it weren't for",'on board','out of the running',"nebulous",'for the sake of'],
    m:['もし～がなかったら','賛成','競争に敗れる','漠然としている','～のために']},
    {d:'254:May 8', 
    v:['But for','evaluate ','intend to',"partake in",'hardle'],
    m:['If it were not for ','assess','plan on','join in/participate in','obstacle']},
    {d:'255:May 11', 
    v:['emulate','tally up','factor into',"feisty",'nerve-wracking'],
    m:['手本にする','集計する','考慮に入れる',' 気骨のある, 元気［威勢］のよい, 物おじしない','神経がすりきれる～']},
    {d:'256:May 14', 
    v:['side gig','preliminary','the name of the game',"work on",'pass on'],
    m:['副業','事前の','～が重要なこと','improveするために動く、働く','パスする、遠慮しておく']},
    {d:'257:May 14', 
    v:['hidden spot','unkept','turn the corner',"(play) the waiting game",'regroup'],
    m:['隠れ場','整理されていない、手入れされていない','峠を越す、危機を脱する','待つだけ','気持ちを入れ替える']},
    {d:'258:May 16', 
    v:['stock up on','middle ground','in/by leaps and bounds',"put aside",'be rattled'],
    m:['必要以上にStockする','妥協点','急速に','取っておく','upset']},
    {d:'259:May 18', 
    v:['get in ones face',' to a certain degree','unkempt',"call s.o over",'let ones hair down'],
    m:['','','','','']},
    {d:'260:May 19', 
    v:['portray','level-headed','delicasy',"trait",'retaliate'],
    m:['～ように描かれる、描写される','calm and sensible in making judgments or decisions OPP hot-headed','珍味','特性・特徴','復習、リベンジ']},
    {d:'261:May 20', 
    v:['propitious','boast','in the same breath',"across the board",'pull in the reins'],
    m:['好都合','brag','～と同時に E.g.He complimented and criticized her in the same breath.','全体で','take control']},
    {d:'262:May 21', 
    v:['main stream','interpret','ones avarage',"amicable",'be rattled','raise ones voice'],
    m:['主流の・有名どころの','～と訳す','平均的な～','friendly','upset','声を荒げる']},
    {d:'263:May 22', 
    v:['brag','be beside oneself','tit for tat',"one of the these days",'bluff'],
    m:['','','','','']},
    {d:'264:May 23', 
    v:['dwell on','brush up(on)','in need',"sink in",'all corner'],
    m:['~に居る、過去にとらわれているなどなど','to practise and improve your skills or your knowledge of something that you learned in the past','困っている','if information, facts etc sink in, you gradually understand them or realize their full meaning E.g. He paused a moment for his words to sink in.','～中の(世界中のみたいなかんじ in all corners of countries)']},
    {d:'265:May 24', 
    v:['be credited with','pliable','big thing',"check up on",'despise','in ones presence'],
    m:['功績・実績などが認められている','使いがってがいい、丈夫','fad','誰かを確認する','hate','誰かがいる前']},
    {d:'266:May 27', 
    v:['intrusive','preferably','heavy weight/light weght',"dry",'feel suffocated'],
    m:['affecting someone’s private life or interrupting them in an unwanted and annoying way','希望としては','酒が強い・弱い','禁酒','息が詰まる']},
    {d:'257:May 14', 
    v:['','','',"",''],
    m:['','','','','']},
  ];

  // *****----End of Vocabraly----*****

  const numbers = [];
  for (let i = 0; i < 266; i++){
    numbers[i] = i + 1;
  }

  let currentnum = 0;
  let isWordson = false;
  const pickcontainer = document.getElementById('pickcontainer');
  const number = document.getElementById('number');
  const words = document.getElementById('words');
  const today = new Date();
  const btn = document.getElementById('btn');
  const meanscontainer = document.getElementById('meanscontainer');
  const means = document.getElementById('means');

  function setWords(){
    while(words.firstChild){
      words.removeChild(words.firstChild);
    }

    const info = document.getElementById('info');
    const date = document.getElementById('date');
    info.textContent = vocaset[currentnum].d;
    date.textContent = `${today.getFullYear()}/${today.getMonth() + 1}/${today.getDate()}`
    const getVoca = [...vocaset[currentnum].v];
    
    getVoca.forEach(eachVoca => {
      const li = document.createElement('li');
      li.textContent = eachVoca;
      
      words.appendChild(li);
    });
  }

  function setMeans(){
    while(means.firstChild){
      means.removeChild(means.firstChild);
    }

    const getMean = [...vocaset[currentnum].m]
    getMean.forEach(mean => {
      const dif = document.createElement('li')
      dif.textContent = mean;

      means.appendChild(dif);
    });
  }

  pickcontainer.addEventListener('click', () =>{
    currentnum = Math.floor(Math.random() * numbers.length);
    number.textContent = currentnum;
    btn.classList.remove('disabled');

    setWords();
    setMeans();
  });

  btn.addEventListener('click', () =>{
    if(btn.classList.contains('disabled')){
      return;
    }

    if (meanscontainer.classList.contains('hidden')){
      meanscontainer.classList.remove('hidden');
      btn.textContent = 'Close';
    }else{
      meanscontainer.classList.add('hidden');
      btn.textContent = 'Show difinitions';
    }
  });

}